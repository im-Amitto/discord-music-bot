...
#######################################################################################################################
    ...
    async def cmd_jump(self, player, channel, author, message, permissions, voice_channel, param=''):
        if not param:
            raise exceptions.CommandError("No param given.", expire_in=20)

        index = int(param)

        if index > len(player.playlist.entries):
            log.warning('length error')

        if player.is_stopped:
            log.warning('player stopped')
        ...
        player.jump(index - 1)
        return Response(
                self.str.get('cmd-skip-reply-skipped-1', '`{0}` times skipped.\n{1}').format(
                    index,
                    self.str.get('cmd-skip-reply-skipped-2', ' Next song coming up!') if player.playlist.peek() else ''
                ),
                reply=True,
